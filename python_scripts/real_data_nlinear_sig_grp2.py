...
# in real data, whether the sig ROIs of both hemishperes are sig 
# in a whole
...
def _run_main_fn(lam, N, setting, is_save=False, is_cv=False, verbose=2):
    ...
    _setting.lam = lam
    _setting.N = N
    ex_idxs = np.concatenate([sig_roi_idxs_1s,  sig_roi_idxs_1s+34])
    _setting.sel_idx = np.delete(np.arange(_setting.data_params.d), ex_idxs)
    
    
    f_name = f"hbroi_grp2-lam_{lam*1000:.0f}-N_{N:.0f}_fit.pkl"
...
sig_roi_idxs_1s = np.setdiff1d(np.array(list(set([idx for idx in sig_roi_idxs if idx <34] +  
...
all_coms = setting.can_lams
with Parallel(n_jobs=35) as parallel:
    ress = parallel(delayed(_run_main_fn)(lam=lam, N=args.N, setting=setting, is_save=True, is_cv=True, verbose=1) for lam
                    in tqdm(all_coms, total=len(setting.can_lams)))
